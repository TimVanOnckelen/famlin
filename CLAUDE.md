...
**Data model** (`backend/prisma/schema.prisma`): `User`, `Setting`, `Group`, `GroupMember` (M2M join, `@@unique([groupId, userId])`), `Post` (belongs to exactly one `Group`), `Comment`, `Like` (a reaction — `type: ReactionType` is `LIKE`/`LOVE`/`HAHA`/`WOW`/`SAD`/`CARE`, one row per user per post/comment via `@@unique([postId, userId])`/`@@unique([commentId, userId])`), `Favorite` (a user's bookmarked posts, `@@unique([postId, userId])`, mirrors `Like`), `PushToken`, `Notification`, `Invite`. `Post`, `Comment`, and `User` are all hard-deleted — `DELETE /api/posts/:id`, `DELETE /api/comments/:id`, and `DELETE /api/admin/users/:id` permanently remove the row via `prisma.*.delete()`, with cascades doing the rest (deleting a post removes its comments/likes/favorites/notifications; deleting a user removes their posts/comments/likes/favorites/etc. too). There is no restore. `User.tokenVersion` is embedded in every JWT and checked on each request, so bumping it (password change/reset) invalidates every previously-issued token immediately. `Notification.type` is a plain string (not a DB enum) — the closed set of values (including `mention` and `on_this_day`) lives in `NotifyType` in `services/notifications.ts`; both currently reuse the `emailOnNewComment`/`pushOnNewComment` and `emailOnNewPost`/`pushOnNewPost` preference columns respectively rather than adding dedicated ones.
